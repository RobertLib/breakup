...
DEPS := $(OBJECTS_REL:.o=.d) $(OBJECTS_DBG:.o=.d) $(OBJECTS_TEST:.o=.d)
-include $(DEPS)
...
$(OBJECTS_REL) $(OBJECTS_DBG) $(OBJECTS_TEST): | $(EMBEDDED_HEADER)
...
# ran embed_assets.sh, which xxd'd 25 MB of Ogg Vorbis into a 159 MB header and
...
#
# `release` and `debug` are this file's own. The other four are what
# packaging/build_*.sh get by overriding BUILD_DIR on the command line; they
# are listed here so that `make clean` means the same thing whichever build
# left something behind.
OBJECT_DIRS := $(BUILD_DIR)/release $(BUILD_DIR)/debug $(BUILD_DIR)/test \
...
# everything.
clean:
	rm -rf $(OBJECT_DIRS) $(TARGET) $(TARGET)_dbg $(TEST_TARGET) $(EMBEDDED_HEADER)
...
.PHONY: all debug release run test embed clean sdlclean distclean mac win linux web press sdl3
