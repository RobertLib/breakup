...
# The slice of libsdl.org's mingw archive to link against. Those archives carry
# i686-w64-mingw32 and x86_64-w64-mingw32 and nothing else, so a UCRT toolchain
# (`MINGW_TRIPLE=x86_64-w64-mingw32ucrt`, which is what Ubuntu's package
# calls it) links the same import libraries as the default one — SDL hands no
# libc object across the DLL boundary, so which C runtime the exe uses is the
# exe's business.
sdl_triple=${SDL_MINGW_TRIPLE:-${triple%ucrt}}
...
for lib in "${SDL3_LIBS[@]}"; do
    ...
    prefix=$repo_dir/$lib-$lib_version/$sdl_triple
    ...
    if [ ! -f "$prefix/lib/lib$lib.dll.a" ]; then
        ...
        if [ ! -f "$prefix/lib/lib$lib.dll.a" ]; then
            echo "build_windows: no $sdl_triple slice in $archive" >&2
            exit 1
        fi
    fi
