...
// How many boss fights bossDefs[] in src/boss/boss.c defines. The table is
// static to that file and has no public count, so this is the number the README
// documents ("N is 1 to 7") - and startBoss() clamps anything above it to the
// last entry, which means a level asking for boss 8 silently gets THE SOVEREIGN
// rather than a complaint. Bump it when an entry is added to the table.
#define BOSS_DEFS_DOCUMENTED 7

// The tree as shipped: twenty-seven files, twenty hand-designed levels in four
// worlds of five and seven boss fights between them. The 1-based file numbers
// that carry a `#boss` line, from the README.
#define SHIPPED_LEVEL_COUNT 27
static const int shippedBossFiles[] = {4, 8, 12, 16, 20, 24, 27};
...
static void testLevelsLoad(void)
{
  ...
  for (int i = 0; i < count; i++)
  {
    ...
    // "LEVEL" is what the parser writes for a file with no `#` line - every
    // level in the tree has one, so seeing the fallback here means a name went
    // missing (or a `# Boss ...` line was mistaken for the directive).
    CHECK(strcmp(level->name, "LEVEL") != 0);
    ...
    // An ordinary level is 0 and a boss fight is an index into bossDefs[],
    // which startBoss() clamps rather than checks - so a number past the end
    // of the table would load and quietly fight the wrong boss.
    CHECK(level->boss >= 0 && level->boss <= BOSS_DEFS_DOCUMENTED);
  }
}
...
static void testWorldAssignment(void)
{
  ...
  // The shape the game is designed around, asserted rather than assumed: this
  // used to be guarded by `if (regular == 20)` and a tree with a different
  // number of ordinary levels skipped it in silence. Adding a twenty-first
  // level is allowed - assignWorlds() piles the extras into the last world on
  // purpose - but it is a change to the game's shape and it should be seen
  // here, not discovered in the level select grid.
  CHECK_INT(regular, WORLD_COUNT * LEVELS_PER_WORLD);
...
static void testMusicMapping(void)
{
  ...
  for (int world = 0; world < WORLD_COUNT; world++)
  {
    ...
    // Every world has to have levels in it and use its whole set; a world
    // with none would have passed here as long as this was `if (distinct > 0)`.
...
// The raw text of one shipped level file, or NULL when the build has not got it
// embedded. The path is the one loadLevelFile() in level-manager.c asks for.
static const EmbeddedAsset *shippedLevelText(int index)
{
  char path[64];

  snprintf(path, sizeof(path), "assets/levels/level%02d.txt", index + 1);

  return findEmbeddedAsset(path);
}

static void testShippedShape(void)
{
  TEST_GROUP("shipped levels: 27 files, 15 cells a row, bosses where the README says");

  const int count = getNumberOfLevels();

  // The tree as shipped. This is the one group that is about these files
  // rather than about any set of files: when a level is added, this number and
  // shippedBossFiles[] above are what to update.
  CHECK_INT(count, SHIPPED_LEVEL_COUNT);

  // Every row of the raw text is exactly LEVEL_PATTERN_COLS wide. The parser
  // pads a short row and drops the tail of a long one without a word, so a
  // level with a fourteen-cell row is a level with a brick missing that
  // nothing but this would ever notice. One check per level, not per row.
  for (int i = 0; i < count; i++)
  {
    const EmbeddedAsset *asset = shippedLevelText(i);
    ...
    const char *text = (const char *)asset->data;
    ...
    CHECK_INT(badRows, 0);
    CHECK(rows > 0);
    CHECK(rows <= LEVEL_PATTERN_ROWS);
  }

  // Names are unique: the level select grid and the high score table both
  // identify a level by nothing else.
  for (int i = 0; i < count; i++)
  {
    ...
    for (int j = 0; j < i; j++)
    {
      if (strcmp(getLevel(i)->name, getLevel(j)->name) == 0)
      {
        duplicate = true;
      }
    }
  ...
  // The boss fights sit exactly where the README puts them - after each world
  // of five, and one more at the very end - and nowhere else.
  for (int i = 0; i < count && i < SHIPPED_LEVEL_COUNT; i++)
  {
    bool expectBoss = false;

    for (size_t k = 0; k < sizeof(shippedBossFiles) / sizeof(shippedBossFiles[0]); k++)
    {
      if (shippedBossFiles[k] == i + 1)
      {
        expectBoss = true;
      }
    }

    CHECK_INT(isBossLevel(i), expectBoss);
  }
}
