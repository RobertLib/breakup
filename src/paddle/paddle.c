...
static void reset(void)
{
  ...
  // Bolts in flight do not carry into the next life or the next level.
  for (int i = 0; i < NUMBER_OF_BULLETS; i++)
  {
...
static void respawnOrGameOver(void)
{
  if (paddle.lives > 1)
  {
    ...
    // The level may have emptied while the paddle was dying; the check waits
    // for the life to be settled, so it is asked again now.
    checkLevelComplete();
  }
  else
  {
