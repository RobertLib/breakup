...
// Parses one level file's text (`size` bytes, not necessarily NUL-terminated)
// into `level`: a '#' line is the name or the `#boss N` directive, everything
// else is pattern cells. This is the parser the loader uses, exposed so it can
// be tested without a file. False for a NULL argument.
