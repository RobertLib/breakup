...
// Whether MIX_Init() succeeded, so that shutdownGame() only undoes what was done.
static bool mixReady;
...
static void runFrame(void)
{
  ...
  // Calculate delta time (clamped to avoid tunneling after hitches). Read in
  // nanoseconds: the millisecond clock this used rounded a 16.7ms frame to 16
  // or 17, which is a 6% wobble in how far the world moved from one frame to
  // the next.
  startTime = SDL_GetTicksNS();
  realDt = (double)(startTime - lastTime) / (double)SDL_NS_PER_SECOND;
  lastTime = startTime;
  ...
  if (!scripted)
  {
    Uint64 elapsed = SDL_GetTicksNS() - startTime;
    Uint64 targetTime = (Uint64)SDL_NS_PER_SECOND / FPS;
    if (targetTime > elapsed)
    {
      SDL_DelayNS(targetTime - elapsed);
    }
  }
#endif
...
int main(UNUSED int argc, UNUSED char *argv[])
{
  ...
  //
  // The web build asks for two more flags, and both of them are about the frame
  // the page gives the canvas rather than about anything the game draws. See
  // packaging/web_shell.html, which is the other half of this.
  //
  //   SDL_WINDOW_RESIZABLE  The canvas is CSS-sized to fill its frame, and on
  //                         itch.io that frame changes size under the game: the
  //                         fullscreen button there calls requestFullscreen()
  //                         on the iframe holding the page, so the whole
  //                         viewport goes from 800x600 to the size of a
  //                         display. SDL's emscripten backend only re-reads the
  //                         canvas's CSS size - and so only resizes the drawing
  //                         buffer to match - for a window that is resizable,
  //                         fullscreen or filling the document
  //                         (Emscripten_HandleResize in
  //                         SDL_emscriptenevents.c). Without this the buffer
  //                         stays the size it was created at and the browser
  //                         stretches it, which is the letterbox arithmetic
  //                         being done twice and neither time right.
  //
  //   SDL_WINDOW_OPENGL     Nothing here wants a GL context of its own: this is
  //                         here to stop SDL_CreateRenderer() throwing the
  //                         window away. The emscripten renderer is opengles2,
  //                         and a window that does not already carry this flag
  //                         is destroyed and made again with it
  //                         (SDL_ReconfigureWindow, in GLES2_CreateRenderer).
  //                         That costs nothing anywhere else, but on this
  //                         platform the teardown writes `width: 1px; height:
  //                         1px` into the canvas's inline style - the one line
  //                         of cleanup SDL can do to an element it is not
  //                         allowed to delete - and the window made next probes
  //                         the canvas's CSS size to decide whether the page is
  //                         sizing it. It measures the 1x1 it was just left,
  //                         concludes that nothing is, takes the CSS size over
  //                         and pins it to 800x600 with an inline style of its
  //                         own. An inline style beats the page's stylesheet,
  //                         so the canvas then stays 800 by 600 in the corner of
  //                         every fullscreen for the rest of the run. It only
  //                         bites where the device pixel ratio is not 1 - every
  //                         one of those writes is inside an `if (pixel_ratio
  //                         != 1.0f)` - which is to say on exactly the Retina
  ...
  window = SDL_CreateWindow(
      ...
      SDL_WINDOW_HIGH_PIXEL_DENSITY | SDL_WINDOW_RESIZABLE | SDL_WINDOW_OPENGL);
#else
