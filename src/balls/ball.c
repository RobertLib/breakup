...
static void handleBossCollision(Ball *ball, float nextX, float nextY)
{
  ...
  if (!bossCollide(nextX + BALL_SIZE / 2.0f, nextY + BALL_SIZE / 2.0f,
  ...
  // A hit on the boss counts towards the combo the same way a brick does; it is
  // the only thing to hit on a boss level.
