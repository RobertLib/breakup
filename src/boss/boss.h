...
// True when this level has a boss and it is still up - taking hits, moving,
// attacking. Goes false on the killing blow, before the death sequence plays.
bool bossAlive(void);

// True while the boss keeps the level open: alive, or still going up. On a boss
// level the bricks are the arena rather than the objective, and an arena brick
// that breaks during the death sequence must not end the level ahead of it.
bool bossBlocksLevelEnd(void);
...
// tenth of a second.
//
// What a ball touched, if anything. Only BOSS_HIT_CORE did the boss any harm.
typedef enum BossHit
{
  BOSS_HIT_NONE = 0,
  BOSS_HIT_ARMOR, // bounced off a plate
  BOSS_HIT_SHUT,  // bounced off the crystal while it was invulnerable
  BOSS_HIT_CORE   // landed on the crystal
} BossHit;

// Returns BOSS_HIT_NONE when nothing was touched. Otherwise *outNormal is the
// unit surface normal to reflect off, and *outPush is how far along it the ball
// has to move to be clear of the surface.
BossHit bossCollide(float cx, float cy, float radius, Vec2 vel,
