...
{
  return boss.present && (boss.alive || boss.dyingT < DEATH_DURATION);
}
                    ...
                    Vec2 *outNormal, float *outPush)
{
  ...
  *outPush = reach - sqrtf(distSq);
  ...
  if (boss.invuln > 0)
  {
    ...
    return BOSS_HIT_SHUT;
  }

  hitCore((Vec2){cx, cy});

  return BOSS_HIT_CORE;
}
...
static void updateDying(void)
{
  ...
  // Bounded by the sequence's end: the level-complete banner runs for a couple
  // of seconds after it, and without the bound this kept detonating under it.
  if (before < DEATH_DURATION &&
      floorf(boss.dyingT / interval) > floorf(before / interval))
  {
