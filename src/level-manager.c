...
// The most a level file on disk may be. A level is 150 rows of 15 cells plus a
// name, a few kilobytes at the outside; the fallback below used to read
// whatever size it found.
#define LEVEL_FILE_MAX_SIZE (64 * 1024)
...
static void assignWorlds(void)
{
  ...
  for (int i = 0; i < numLevels; i++)
  {
    ...
    // Counted from the first level of its world - which, once the worlds have
    // run out, is the first level of the last one, so a twenty-first regular
    // level is W4-6 and not a second W4-1. Below that point it is the old
    // `regular % LEVELS_PER_WORLD + 1` exactly.
    levelNumberInWorld[i] = regular - world * LEVELS_PER_WORLD + 1;
    regular++;
  }
}
...
bool parseLevelText(Level *level, const char *text, size_t size)
{
  ...
  return parseLevelFromText(level, text, size);
}
...
static bool loadLevelFile(int index)
{
  ...
  if (fileSize > LEVEL_FILE_MAX_SIZE)
  {
    fprintf(stderr, "%s is %lld bytes; a level file may be at most %d\n",
            filename, (long long)fileSize, LEVEL_FILE_MAX_SIZE);
    SDL_CloseIO(file);
    return false;
  }
